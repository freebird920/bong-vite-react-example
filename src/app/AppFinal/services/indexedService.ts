export class IndexedService {
    ...
    public addData(storeName: string, data: any): Promise<string> {
      return new Promise((resolve, reject) => {
        ...
        request.onerror = (event: Event) => reject(`데이터 추가 오류: ${(event.target as IDBRequest).error?.message}`);
      });
    }
    ...
    public getData(storeName: string, id: number | string): Promise<any> {
      return new Promise((resolve, reject) => {
        ...
        request.onerror = (event: Event) => reject(`데이터 조회 오류: ${(event.target as IDBRequest).error?.message}`);
      });
    }
    ...
    public deleteData(storeName: string, id: number | string): Promise<string> {
      return new Promise((resolve, reject) => {
        ...
        request.onerror = (event: Event) => reject(`데이터 삭제 오류: ${(event.target as IDBRequest).error?.message}`);
      });
    }
  }